...
module.exports = {
    ...
    Essaii : function(req,res)
    {
      ...
      var cheminp = [];
      var MotCle= [];
      workbook.xlsx.readFile('Retour.xlsx')
          .then(function() {
            ...
            var motcle = newworksheet.getColumn(10);
              ...
              async.series([  
                 ...
                 function(cb){
                      ReportingRetour.importEssai(table,cheminp,date,MotCle,0,cb);
                    },
                 /*function(cb){
                      ReportingRetour.importEssai(table,cheminp,date,MotCle,1,cb);
                    },
                    /*function(cb){
    ...
    {
      var sql= 'select * from cheminretour limit 1;';
                ...
                .then(function() {
                  ...
                  var numeroligne = newworksheet.getColumn(8);
                    ...
                    async.series([
                      ...
                      function(cb){
                        ReportingRetour.importTrameFlux929(trameflux,feuil,cellule,table,cellule2,nb,numligne,cb);
                      }, 
                      /*function(cb){
                            ReportingRetour.deleteTout(table,nb,cb);
                          }, 
                       function(cb){
                          ReportingRetour.deleteHtp(table,nb,cb);
                        }, 
                     function(cb){
                          ReportingRetour.importInovcom(trameflux,feuil,cellule,table,cellule2,numligne,nb,cb);
                          },
                     function(cb){
                        ReportingRetour.importTout(trameflux,table,nb,cb);
                        }, */
                    ],
                    function(err, resultat){
                      if (err) { return res.view('Retour/erreur'); }
